...
<liferay-portlet:renderURL varImpl="upsertURL" var="upsertURL" >
	<liferay-portlet:param name="jspPage" value="/html/upsert.jsp" />
...
<aui:form action="<%=searchURL %>" method="post" name="fm">
	...
	<aui:nav-bar>
		<aui:nav>
			<aui:nav-item href="${upsertURL }" label="create-a-remote-application-form" title="create-a-remote-application-form" iconCssClass="icon-plus-sign" name="addButton" />
			
			<aui:nav-item dropdown="true" label='<%=LanguageUtil.get(locale, "cycle") + ": " + cycle %>' iconCssClass="icon-calendar">
				<%
				List<String> cycles = RemoteFormLocalServiceUtil.fetchCycles(applicant, WorkflowConstants.STATUS_APPROVED);
				for (String c : cycles) {
				%>
...
<liferay-ui:search-container emptyResultsMessage="no-entries-were-found" iteratorURL="<%=iteratorURL %>" orderByCol="<%=orderByCol %>" orderByType="<%=orderByType %>">
	...
	<liferay-ui:search-container-results>
		...
		results = RemoteFormLocalServiceUtil.search(applicant, WorkflowConstants.STATUS_APPROVED, startDateTime, endDateTime, keywords, searchContainer.getStart(), searchContainer.getEnd(), obc);
		total = RemoteFormLocalServiceUtil.countBySearch(applicant, WorkflowConstants.STATUS_APPROVED, startDateTime, endDateTime, keywords);
...
<liferay-portlet:renderURL varImpl="changeCycleURL">
	...
	<liferay-portlet:param name="tabs1" value="approved" />
	<liferay-portlet:param name="orderByCol" value="<%=orderByCol %>" />
	<liferay-portlet:param name="orderByType" value="<%=orderByType %>" />
	<liferay-portlet:param name="keywords" value="<%=keywords %>" />
