	...
	<aui:nav-bar>
		<aui:nav>		
			<aui:nav-item dropdown="true" label='<%=LanguageUtil.get(locale, "cycle") + ": " + cycle %>' iconCssClass="icon-calendar">
				<%
				List<String> cycles = RemoteFormLocalServiceUtil.fetchCycles(applicant, WorkflowConstants.STATUS_PENDING);
				for (String c : cycles) {
				%>
...
<liferay-ui:search-container emptyResultsMessage="no-entries-were-found" iteratorURL="<%=iteratorURL %>" orderByCol="<%=orderByCol %>" orderByType="<%=orderByType %>">
	...
	<liferay-ui:search-container-results>
		...
		results = RemoteFormLocalServiceUtil.search(applicant, WorkflowConstants.STATUS_PENDING, startDateTime, endDateTime, keywords, searchContainer.getStart(), searchContainer.getEnd(), obc);
		total = RemoteFormLocalServiceUtil.countBySearch(applicant, WorkflowConstants.STATUS_PENDING, startDateTime, endDateTime, keywords);
...
<liferay-portlet:renderURL varImpl="changeCycleURL">
	...
	<liferay-portlet:param name="tabs1" value="pending" />
	<liferay-portlet:param name="orderByCol" value="<%=orderByCol %>" />
	<liferay-portlet:param name="orderByType" value="<%=orderByType %>" />
	<liferay-portlet:param name="keywords" value="<%=keywords %>" />
