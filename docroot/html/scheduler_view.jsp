<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
	<%@ include file="/html/init.jsp" %>
		...
		<% String tabs1=ParamUtil.getString(request, "tabs1" ); String
			applicant=ParamUtil.getString(request, "applicant" , user.getScreenName()); Calendar
			cal=Calendar.getInstance(); cal.add(Calendar.DAY_OF_YEAR, -180); Date searchDateStart=cal.getTime(); long
			searchDateStartValue=searchDateStart.getTime(); cal.add(Calendar.DAY_OF_YEAR, 240); Date
			searchDateEnd=cal.getTime(); long searchDateEndValue=searchDateEnd.getTime(); String[]
			colorSet={"#FF99CC", "#0099FF" , "#00FF99" , "#9933FF" , "#99CC99" , "#CC9900" , "#CCFF99" , "#88DD88"
			, "#FF6699" , "#FFCC33" , "#66CCFF" , "#CCCC00" , "#99CCCC" , "#996666" , "#99FFFF" , "#CC9966" , "#33CCCC"
			, "#9999FF" , "#FF3366" , "#FFCC99" , "#00CCFF" , "#339966" , "#CC0033" , "#FFFF33" , "#FFCC66" , "#FFCCFF"
			, "#FF6666" , "#66CC66" , "#66FFFF" , "#9999CC" , "#555555" , "#AAAAAA" }; List<RemoteForm> remoteForms =
			RemoteFormLocalServiceUtil.searchForStatistics(applicant, new int[] { WorkflowConstants.STATUS_APPROVED,
			WorkflowConstants.STATUS_PENDING }, searchDateStart, searchDateEnd);
			...
			List<Object[]> list = new ArrayList<Object[]>();
					...
					Date endTime = Validator.isNotNull(remoteForm.getEndDateTime()) ? remoteForm.getEndDateTime() :
					MonthDateRange.fullTime(startTime);
					boolean allDay = Validator.isNotNull(remoteForm.getDayTab()) ?
					!remoteForm.getDayTab().equals(RemoteFormConstants.DAY_TABS.THAT_DAY) : true;

					list.add(new Object[]{remoteForm.getRemoteFormId(), content , startTime, endTime, colorSet[0],
					allDay });
							...
							<aui:row>
								<aui:col cssClass="text-center" span="2">
									<aui:select name="applicant" label="applicant" cssClass="span12">
										<% String keywords=StringPool.BLANK; int
											workStatus=Constants._WORKING_STATUS.WORKING; OrderByComparator
											obc=OrderByComparatorFactoryUtil.create("ee", "employeeKey" , false);
											Set<Employee> employees = new LinkedHashSet<Employee>();

												if (permissionChecker.isOmniadmin()) {
												employees.addAll(EmployeeLocalServiceUtil.search(workStatus, keywords,
												QueryUtil.ALL_POS, QueryUtil.ALL_POS, obc));

												} else {
												Employee curEmployee =
												EmployeeLocalServiceUtil.fetchByLRUserId(user.getUserId());

												if (Validator.isNotNull(curEmployee)) {
												employees.add(curEmployee);

												int level = 3;
												employees.addAll(EmployeeLocalServiceUtil.getEmployeeHierarchy(curEmployee,
												level, new LinkedList<Employee>()));
													}
													}

													for (Employee employee : employees) {
													String lrUserScreenName = Validator.isNotNull(employee.getLRUser())
													? employee.getLRUser().getScreenName() : StringPool.BLANK;
													String applicantLabel = employee.getName_() + StringPool.SLASH +
													employee.getEnglishName();
													%>
													<aui:option value="<%=lrUserScreenName %>"
														label="<%=applicantLabel %>"
														selected="<%=applicant.equals(lrUserScreenName) %>" />
													<% } %>
									</aui:select>
								</aui:col>
								<aui:col width="20">
					...
					<aui:script use="aui-base, dd-drag">
						...
						var events = <%=schedulerScript %>;
							...
							boundingBox: '#
							<portlet:namespace />remoteFormScheduler',
							date: new Date(),
							...
							var portlet = A.one('#p_p_id
							<portlet:namespace />');
							var refreshPortlet = function() { Liferay.Portlet.refresh("#p_p_id
							<portlet:namespace />");};
							portlet.delegate('click', refreshPortlet, '#
							<portlet:namespace />refreshButton');

							var applicantSelector = A.one('#
							<portlet:namespace />applicant');
